...
<div
  ...
  <div class="grid-headers" rolw="rowgroup"
    style="height: {headerHeight}px; margin-top: {rowHeight -headerHeight}px" >
    ...
    <div
      ...
      style="left: -{__scrollLeft}px; height: {headerHeight}px; width: {gridSpaceWidth}px;"
      role="row">
      {#each columns as column, i (i)}
        <div
          class={'grid-cell' + (column.parent ? ' parent' : '') + (column.child ? ' child' : '')}
          ...
          )}px; width: {columnWidths[i]}px; height: {headerHeight}px; line-height: {headerHeight}px;"
...
<style>
  ...
  .grid-row:hover {
    border-bottom-color: rgb(40, 150, 250);
  }
  ...
  .grid-cell.parent {
    ...
    /* border-left: 1px solid rgba(51, 201, 191); */
    padding-left: 0px;
  ...
  .grid-header-row .grid-cell.child {
    border-right: none;
  }
